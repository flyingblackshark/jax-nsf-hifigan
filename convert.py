...
def convert_torch_weights(path = "lynx-combo-opencpop-kiritan-vocoder.ptc"):
    state_dict = torch.load(path,map_location=torch.device('cpu'))["vocoder"]["model"]["generator"]
    params = {}
    ...
    for i in range(5):
        params[f"ups_{i}.layer_instance/kernel/scale"] = state_dict[f"ups.{i}.weight_g"].squeeze((1, 2))
        params[f"ups_{i}.layer_instance.kernel"] = state_dict[f"ups.{i}.weight_v"].T
        params[f"ups_{i}.layer_instance.bias"] = state_dict[f"ups.{i}.bias"]
    for i in range(15):
